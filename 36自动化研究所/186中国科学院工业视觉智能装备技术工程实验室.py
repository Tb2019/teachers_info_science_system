...
laboratory_name = '中国科学院工业视觉智能装备技术工程实验室'
institute_id = 36
laboratory_id = 186
img_url_head = None
partition_num = '010'
start_urls = [
                'http://ivi.ia.ac.cn/yjy',
                'http://ivi.ia.ac.cn/fyjy',
                'http://ivi.ia.ac.cn/zlyjy',
                'http://ivi.ia.ac.cn/gcs'
              ]

a_s_xpath_str = '//ul[@class="w-list-ul"]/li/a'
target_div_xpath_str = '//div[@class="yibuFrameContent tem_32_15  area_Style1  "]'
...
class SpecialSpider(ReCrawler):
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            name = a.xpath('./div[@class="w-list-bottom"]/h5/text()')
            if name:
                name = ''.join(name)
