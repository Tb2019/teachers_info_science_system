...
from utils import csv_header, result_dict_2_df, csv_2_df, drop_duplicate_collage, truncate_table, df2mysql, \
    local_engine, sf_engine, save_as_json, get_response_async, api_parse, replace_quotes_in_text, clean_phone
...
institute_name = '深海科学与工程研究所'
laboratory_name = '深海科学与工程研究所'
institute_id = 96
laboratory_id = 345
img_url_head = None
partition_num = '0898'
start_urls = [
                # 'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_yjy/',
                # 'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_yjy/index_1.html',
                # 'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_yjy/index_2.html',
                'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_yjy/index_3.html',
                'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_fyjy/',
                'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_fyjy/index_1.html',
                'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_fyjy/index_2.html',
                'https://idsse.cas.cn/rcdw2015/yjxl2015/yjxl_fyjy/index_3.html'
              ]

a_s_xpath_str = '//ul[@class="ds"]/li'
target_div_xpath_str = '//div[@id="detailcenter"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            name = a.xpath('.//h3[@class="fs18 fw1 ffw"]/a/text()')
            if name:
                name = ''.join(name)
                ...
                phone = ''.join(a.xpath('./div[@class="jies fr"]/div[2]/text()'))
                phone = re.sub(r'\s*电话：\s*', '', phone)
                phone = re.sub(r'\s', '', phone)
                if not phone:
                    phone = None
                phone = clean_phone(partition_num, phone)

                email = ''.join(a.xpath('./div[@class="jies fr"]/div[4]/text()'))
                email = re.sub(r'\s*电子邮箱：\s*', '', email)
    ...
    def get_detail_page(self, index_result):
        ...
        tasks = [get_response_async(url, session, name=name, phone=phone, email=email) for name, url, phone, email in index_result if len(name) >= 2]
    ...
    def get_detail_page(self, index_result):
        ...
        for name, url, phone, email in index_result:
            if len(name) >= 2:
                try:
                    driver.get(url)
                    time.sleep(1)
                    page = driver.page_source
                    detail_pages.append((page, url, [('name', name), ('phone', phone), ('email', email)]))
                except:
                    pass
        driver.close()
        return detail_pages
