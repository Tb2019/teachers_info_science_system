...
institute_name = '数学与系统科学研究院数学研究所'
laboratory_name = '数学与系统科学研究院数学研究所'
institute_id = 7
laboratory_id = 68
img_url_head = None
partition_num = '010'
start_urls = [
                'http://www.math.ac.cn/people/',
                # '',
                # '',
                # ''
              ]

a_s_xpath_str = '//div[@class="martop15 gailan_list paddbottom10"]/div[3]/div//tbody'
...
class SpecialSpider(ReCrawler):
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            ...
            if name:
                ...
                link = a.xpath('./tr[2]//a/@href')[0]
                link = parse.urljoin(url, link)
                img = a.xpath('./tr[1]//img/@src')[0]
                mail = re.sub(r'\s', '', ''.join(a.xpath('./tr[4]//text()')))
                direc = re.sub(r'\s', '', ''.join(a.xpath('./tr[5]//text()')))
            else:
                print('未解析到name，请检查：a_s_xpath_str')
                continue
            yield name, link, mail, direc, img
    ...
    def get_detail_page(self, index_result):
        ...
        tasks = [get_response_async(url, session, name=name, mail=mail, direc=direc, img=img) for name, url, mail, direc, img in index_result if len(name) >= 2]
