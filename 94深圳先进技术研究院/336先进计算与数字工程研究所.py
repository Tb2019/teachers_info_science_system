...
institute_name = '深圳先进技术研究院'
laboratory_name = '先进计算与数字工程研究所'
institute_id = 94
laboratory_id = 336
img_url_head = None
partition_num = '0755'
start_urls = [
                'https://szs.siat.ac.cn/cms/api/content/getChildPageList?chineseOrEnglish=0&organizationId=1&isPc=1&pageNumber=1&pageSize=100&menuId=61',
                'https://szs.siat.ac.cn/cms/api/content/getChildPageList?chineseOrEnglish=0&organizationId=1&isPc=1&pageNumber=1&pageSize=100&menuId=62',
                'https://szs.siat.ac.cn/cms/content/queryIndexContent?chineseOrEnglish=0&organizationId=1&isPc=1&menuId=63&fixPidMenuId=63',
                'https://szs.siat.ac.cn/cms/content/queryIndexContent?chineseOrEnglish=0&organizationId=1&isPc=1&menuId=595&fixPidMenuId=595',
              ]

a_s_xpath_str = ''
target_div_xpath_str = '//div[@class="detail-container"]'
...
def ajax_index(url):
    ...
    headers = {
        ...
        'content-length': '0',
        'dnt': '1',
        'origin': 'https://szs.siat.ac.cn',
        'pragma': 'no-cache',
        'priority': 'u=1, i',
        'referer': 'https://szs.siat.ac.cn/',
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        info_s = index_page['data'][0]['list']
        for info in info_s:
            name = info['contentTitle']
            link = 'https://szs.siat.ac.cn/#/detail?item=%5Bobject%20Object%5D&list=false&contentId=' + str(info['id'])
            img = info['mainPic']
            yield name, link, img
    ...
    def get_detail_page(self, index_result):
        ...
        for name, url, img in index_result:
            if len(name) >= 2:
                driver.get(url)
                driver.refresh()
                ...
                detail_pages.append((page, url, [('name', name), ('img', img)]))
        driver.close()
        return detail_pages
